...
public class FlowLimitFilter extends HttpFilter {
    /* TODO: Written by - Han Yongding 2024/04/27 限流过滤器特定时间 */
    @Value("${spring.security.filter.max-request-limit-time}")
    private int maxLimitRequestTime;

    /* TODO: Written by - Han Yongding 2024/04/27 特定时间访问不能超过 */
    @Value("${spring.security.filter.max-request-limit}")
    private int maxLimitRequest;

    /* TODO: Written by - Han Yongding 2024/04/27 违规封禁时间 */
    @Value("${spring.security.filter.ban-time}")
    ...
    private boolean limitPeriodCheck(String ip) {
        ...
        if (Boolean.TRUE.equals(template.hasKey(Const.FLOW_LIMIT_COUNTER + ip))) {
            ...
            /* TODO: Written by - Han Yongding 2023/09/17 如果三秒请求大于三十次，表示已经被封禁 */
            if(increment > maxLimitRequest) {
            ...
            template.opsForValue().set(Const.FLOW_LIMIT_COUNTER + ip, "1", maxLimitRequestTime, TimeUnit.SECONDS) ;
        }
        return true ;
    }
