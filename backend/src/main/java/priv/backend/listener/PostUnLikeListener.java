...
@Component
@Slf4j
@RabbitListener(queues = "postUnLikeRetry")
public class PostUnLikeListener {
    ...
    @Autowired
    public PostUnLikeListener(PostServiceImpl postService,
    ...
    @RabbitHandler
    public void receiveUnPostLikeRetry(RestCountVO vo) {
        ...
        if (postService.deleteLikeByPostIdAndUserId(vo.getId(), vo.getUserId()) != null) {
        ...
        while (!updated && retries < maxRetries) {
            ...
            updated = postService.updateUnLikeAndVersionByPostId(vo);
