...
import {post, get, put, delete_} from "@/net/http";
...
const getData = async (num, size) => {
  ...
  try {
    const response = await new Promise((resolve, reject) => {
      get("api/backend/user-level?pageNum=" + page.value + "&pageSize=" + pageSize.value, (rs) => {
        if (rs.code === 200) {
          resolve(rs);
        } else {
          reject(rs);
...
function deleteStatusData(levelId) {
  if (!(levelId === "") || !(levelId === undefined)) {
    /* 请求后台删除数据 */
    delete_(
        "api/backend/user-level/" + levelId,
        async (rs) => {
          if (rs.code === 200) {
