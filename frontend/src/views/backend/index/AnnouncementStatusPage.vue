...
function openDrawer(data) {
  ...
  if (data.type === 1) {
    ...
    drawerTitle = "编辑公告状态";
    /*修改*/
    updateStatus(data.statusId, data.statusName, data.createId);
  }
...
/* 修改，为表单内容赋值 */
function updateStatus(statusId, statusName, createId) {
...
const getData = async (num, size) => {
  ...
  try {
    const response = await new Promise((resolve, reject) => {
      get("api/auth/announcement-status?pageNum=" + page.value + "&pageSize=" + pageSize.value, (rs) => {
        if (rs.code === 200) {
          resolve(rs);
        } else {
          reject(rs);
...
const putState = (data) => {
  return new Promise((resolve, reject) => {
    put("/api/auth/announcement-status", data,
        () => {
...
function deleteStatusData(statusId) {
  if (!(statusId === "") || !(statusId === undefined)) {
    /* 请求后台删除数据 */
    delete_(
        "/api/auth/announcement-status/" + statusId,
        async (rs) => {
          if (rs.code === 200) {
...
#announcement_status >>> #top {
  padding-left: 10px;
  padding-right: 10px;
  //margin: 10px ;
}

#announcement_status >>> #bottom {
  padding-left: 10px;
  padding-right: 10px;
  padding-bottom: 10px;
}

#announcement_status >>> #buttons {

}

#announcement_status >>> #tables {
  margin-top: 20px;
}

#announcement_status >>> .title {
  display: block;
  padding-top: 10px;
  font-size: 18px;
}

#announcement_status >>> .form-top {
  margin-top: 25px;
}

#announcement_status >>> .pagination {
  margin-top: 12px;
  display: flex;
