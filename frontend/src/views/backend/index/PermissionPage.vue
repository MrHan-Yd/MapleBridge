...
const getData = async (num, size) => {
  ...
  try {
    const response = await new Promise((resolve, reject) => {
      get("api/auth/permission?pageNum=" + page.value + "&pageSize=" + pageSize.value, (rs) => {
        if (rs.code === 200) {
          resolve(rs);
        } else {
          reject(rs);
...
const editStatus = async (row) => {
  loading = true;
  const statusId = row.statusId === '1750807849758318594' ? '1750807828107321346' : '1750807849758318594';
  try {
    await putState({
      permissionId: row.permissionId,
      statusId
        ...
        <el-table :data="tableData" :row-key="'permissionId'" :height="'53vh'"
          ...
          <el-table-column prop="statusId" label="状态" width="120">
            <template #default="scope">
              <el-switch
                  :loading="loading"
                  :model-value="scope.row.statusId === '1750807828107321346'"
                  :checked-value="0"
                  :unchecked-value="1"
              ...
              <div>
                ...
                >
                  ...
                  <div style="text-align: right; margin: 0">
                    ...
                    <el-button size="small" type="primary" @click="deleteStatusData(scope.row.permissionId)">确定</el-button>
                  </div>
                  <template #reference>
                    <el-button link type="primary" size="small" @click="showDeletePopover(scope.row.permissionId)">删除</el-button>
                  </template>
                </el-popover>
              </div>
            </template>
      ...
      <div>
        <el-form :model="form" :rules="rule" ref="formRef">
          <el-form-item prop="permissionName">
            <el-input :prefix-icon="Connection" v-model="form.permissionName" placeholder="权限名称" clearable/>
          </el-form-item>
          <el-form-item prop="permissionUrl">
            <el-input :prefix-icon="Link" v-model="form.permissionUrl" placeholder="权限路径" clearable/>
          </el-form-item>
        </el-form>
      </div>
    </template>
